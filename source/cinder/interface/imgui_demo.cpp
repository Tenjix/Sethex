...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
	...
	if (ImGui::TreeNode("Colors")) {
		...
		static constexpr ImGuiColorEditMode ImGuiColorEditMode_RGB_Float = 3;
		...
		ImGui::SameLine();
		ImGui::RadioButton("RGB Float", &edit_mode, ImGuiColorEditMode_RGB_Float);
		...
		for (int i = 0; i < ImGuiCol_COUNT; i++) {
			...
			if (!filter.PassFilter(name))
				continue;
			ImGui::PushID(i);
			if (edit_mode == ImGuiColorEditMode_RGB_Float) {
				ImGui::ColorButton(style.Colors[i]);
				ImGui::SameLine();
				ImGui::DragFloat4(name, (float*)&style.Colors[i], 0.005f, 0.0f, 1.0f, "%.2f");
			} else {
